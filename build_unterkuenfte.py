...
def map_eintrag(t, concepts):
    ...
    # Feratel-UUID aus identifier-Array extrahieren. DataHub liefert in
    # identifier[] ein Array von PropertyValue-Objekten, eines davon mit
    # propertyID='feratel'. Diese UUID identifiziert die Unterkunft im
    # TOSC5-System auf westerwald.info -- darueber koennen wir spaeter
    # direkt auf die Buchungsseite verlinken.
    feratel_uuid = ""
    idents = t.get("identifier") or []
    if isinstance(idents, dict): idents = [idents]
    for it in idents:
        if isinstance(it, dict) and it.get("propertyID") == "feratel":
            feratel_uuid = it.get("value") or ""
            break

    eintrag = {
        "id":          "dh-" + uid,
        "slug":        slug,
        "feratelUuid": feratel_uuid,
        "name":        name,
...
def main():
    ...
    parser.add_argument("--var", default="DATA_UNTERKUENFTE_ALLE",
                        help="window-Variable (Standard: DATA_UNTERKUENFTE_ALLE)")
    ...
    if args.inputs:
        # Windows: CMD und PowerShell expandieren Wildcards (*.json) NICHT
        # automatisch -- Python bekommt den literalen String. Daher selbst
        # auflösen. Funktioniert auf jeder Plattform: nicht-Glob-Pfade werden
        ...
        for p in args.inputs:
            if any(c in p for c in '*?['):
                ...
                if not matches:
                    print("WARN: kein Treffer fuer Muster: " + p, file=sys.stderr)
