...
# -*- coding: utf-8 -*-
"""
build_wandertouren.py — Mapper DataHub-RLP Wandertouren → App-Format
...
'gmw-wandertouren-ww' (braucht --token).

Schreibt eine IIFE-JS-Datei, die sich in window.DATA_WANDERN_ALLE
einklinkt — analog zur veranstaltungen-datahub.js.

Aufruf (lokal):
    python build_wandertouren.py wander-1.json wander-2.json -o wandertouren-datahub.js

Aufruf (API):
    python build_wandertouren.py --token <TOKEN> -o wandertouren-datahub.js
"""
...
# Pattern -> Tour-Reihen-Bezeichnung. Reihenfolge wichtig (spezifischer zuerst).
TOUR_REIHEN_PATTERNS = [
    (re.compile(r"^westerwaldsteig\b",                re.I),  "Westerwaldsteig"),
    (re.compile(r"^druidensteig\b",                   re.I),  "Druidensteig"),
    (re.compile(r"^wiedweg\b",                        re.I),  "Wiedweg"),
    (re.compile(r"^(kleiner|kleine)\s+(wäller|waeller)\b", re.I), "Kleine Wäller"),
    (re.compile(r"^(wäller|waeller)\s+tour\b",        re.I),  "Wäller Touren"),
    (re.compile(r"^r\d+\b"),                                   "Regionale Rundwege"),
]

def detect_tour_reihe(name):
    """Leitet die Tour-Reihe aus dem Namen ab. Falls keine passt: Einzeltour."""
    n = (name or "").strip()
    for pat, label in TOUR_REIHEN_PATTERNS:
        if pat.search(n):
            return label
    return "Einzeltour"
...
def map_tour(t):
    ...
    return {
        ...
        "slug":            slug,
        "titel":           name,
        "tourReihe":       detect_tour_reihe(name),
...
# ---------------------------------------------------------------------------
# Output: IIFE-Datei mit Merge-Logik in window.DATA_WANDERN_ALLE
# ---------------------------------------------------------------------------
...
def schreibe_js(eintraege, output_path):
    ...
    js_code = (
        ...
        "// Anzahl Touren: " + str(len(eintraege)) + "\n"
        "// ════════════════════════════════════════════════════════════════\n"
        "(function() {\n"
        "  if (!window.DATA_WANDERN_ALLE) window.DATA_WANDERN_ALLE = [];\n"
        "  if (window.__DATA_WANDERN_DATAHUB_GELADEN) {\n"
        "    console.warn('[Wandertouren DATAHUB] Datei wurde bereits geladen, ueberspringe Merge.');\n"
        "    return;\n"
        "  }\n"
        "  window.__DATA_WANDERN_DATAHUB_GELADEN = true;\n"
        "\n"
        "  var neu = " + payload_json + ";\n"
        "  var bestand = window.DATA_WANDERN_ALLE;\n"
        "  var vorhanden = {};\n"
        ...
        "    hinzu++;\n"
        "  }\n"
        "  console.log('[Wandertouren DATAHUB] Bestand vorher: ' + vorher\n"
...
def main():
    ...
    parser.add_argument("-o", "--output", default="wandertouren-datahub.js",
                        help="Output-Datei (Standard: wandertouren-datahub.js)")
    parser.add_argument("--endpoint", default="gmw-wandertouren-ww",
                        help="DataHub-Endpoint-Slug (Standard: gmw-wandertouren-ww)")
    ...
    for t in roh:
        ...
        tour_reihen[e["tourReihe"]] = tour_reihen.get(e["tourReihe"], 0) + 1
        eintraege.append(e)

    # Sortiert: zuerst nach Tour-Reihe, innerhalb nach Titel
    eintraege.sort(key=lambda x: (x["tourReihe"], x["titel"].lower()))
    ...
    print("  Tour-Reihen-Verteilung:", file=sys.stderr)
    for r in sorted(tour_reihen.keys()):
        print("    " + r.ljust(28) + str(tour_reihen[r]), file=sys.stderr)
